...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private OnRefreshScrollListener refreshScrollListener = null;

	public void setOnRefreshScrollListener(
			OnRefreshScrollListener refreshScrollListener) {
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			if (refreshScrollListener != null) {
				refreshScrollListener.onScroll();
			}
			if (startTime == -1) {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mode = a.getInteger(R.styleable.PullToRefresh_mode, MODE_BOTH);
		// mode = MODE_BOTH;
		// Refreshable View
		...
		if (a.hasValue(R.styleable.PullToRefresh_headerBackground)) {
			this.setBackgroundResource(a.getResourceId(
					R.styleable.PullToRefresh_headerBackground, Color.WHITE));// WHITE
		}
